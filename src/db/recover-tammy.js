/* =================================================================
   RECOVERY ACCOUNT RESTORE — Tammy Riley (tammy.riley)
   -----------------------------------------------------------------
   Creates (or re-classifies) the recovery/test account and records the
   recovery snapshot + intended plan configuration. It is IDEMPOTENT.

   What this script NEVER does:
     - deletes, truncates or resets any table
     - writes ledger rows for the recovered balance
     - creates deposits / withdrawals / investments / transactions
     - backdates anything and presents it as genuine history

   The $2,048.00 balance and BTC/ETH holdings live ONLY in
   user_balance_snapshots / user_balance_snapshot_assets, which the
   BalanceService renders as recovery_display_balance. The ledger stays
   empty for this account, so nothing can be mistaken for real history.

   Run with:  node src/db/recover-tammy.js
   ================================================================= */
...
async function main() {
  ...
  if (!user) {
    const pw = process.env.TAMMY_PASSWORD || 'Tammy!Recovery2026';
    [user] = await sql`
...
main().catch((e) => { console.error('[recover-tammy] FAILED:', e); process.exit(1); });